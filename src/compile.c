...
static TokenType variableType(TokenType type) {
    if (type == TOKEN_INT) return TOKEN_VAR_INT;
    if (type == TOKEN_FLOAT) return TOKEN_VAR_FLOAT;
    if (type == TOKEN_STRING) return TOKEN_VAR_STRING;
    if (type == TOKEN_BOOL) return TOKEN_VAR_BOOL;
    return type;
}

static const char *cType(TokenType type) {
    if (type == TOKEN_VAR_FLOAT) return "double";
    if (type == TOKEN_VAR_STRING) return "char*";
    if (type == TOKEN_VAR_BOOL) return "bool";
    return "int";
}

static TokenType lookupLocalType(const LocalVariable locals[], int localCount,
                                 const char parameters[][64], const TokenType parameterTypes[],
                                 int parameterCount, const char *name) {
    for (int i = localCount - 1; i >= 0; i--) {
        if (strcmp(locals[i].name, name) == 0) return locals[i].type;
    }
...
static TokenType inferExpressionType(Token *tokens, int start, int end,
                                     const LocalVariable locals[], int localCount,
                                     const char parameters[][64], const TokenType parameterTypes[],
                                     int parameterCount) {
    ...
    for (int i = start; i < end; i++) {
        ...
        if (type == TOKEN_VAR_STRING || type == TOKEN_VAR_BOOL) return type;
        if (type == TOKEN_VAR_FLOAT) result = TOKEN_VAR_FLOAT;
        if (type == TOKEN_VAR_INT && result == TOKEN_KEYWORD) result = TOKEN_VAR_INT;
    }
...
static void emitExpression(Token *tokens, int start, int end, FILE *out) {
    for (int i = start; i < end; i++) {
        ...
        if (strcmp(tokens[i].value, ",") == 0) fprintf(out, " ");
    }
}

static void emitPrint(Token *tokens, int start, int end, TokenType type, FILE *out) {
...
static void inferParameterTypes(Token *tokens, int count, const char *functionName,
...
void generateFunctions(Token *tokens, int count, FILE *out) {
    for (int i = 0; i < count; i++) {
        if (tokens[i].type == TOKEN_FUNC) {
            ...
            inferParameterTypes(tokens, count, functionName, parameterTypes, parameterCount);
            fprintf(out, "void %s(", functionName);
            for (int parameter = 0; parameter < parameterCount; parameter++) {
                if (parameter > 0) fprintf(out, ", ");
                fprintf(out, "%s %s", cType(parameterTypes[parameter]), parameters[parameter]);
            }
            fprintf(out, ") {\n");
            i++;  // Skip ')'
            ...
            while (i < count && strcmp(tokens[i].value, KEYWORD_END) != 0) {
                int lineEnd = i + 1;
                while (lineEnd < count && tokens[lineEnd].line == tokens[i].line) lineEnd++;
                ...
                if (tokens[i].type == TOKEN_EXPOSE_C) {
                    ...
                    if (i + 1 >= lineEnd) {
                        ...
                        TokenType type = inferExpressionType(tokens, i + 1, lineEnd, locals,
                                                             localCount, parameters, parameterTypes,
                                                             parameterCount);
                    ...
                    if (i + 3 >= lineEnd || strcmp(tokens[i + 2].value, "=") != 0) {
                        ...
                        TokenType type = inferExpressionType(tokens, i + 3, lineEnd, locals,
                                                             localCount, parameters, parameterTypes,
                                                             parameterCount);
...
void generate(Token *tokens, int count, FILE *out) {
    ...
    while (i < count) {
        if (tokens[i].type == TOKEN_FUNC) {
            ...
            while (i < count && strcmp(tokens[i].value, ")") != 0) i++;
            if (i < count) i++;  // Skip ')'
            while (i < count && strcmp(tokens[i].value, KEYWORD_END) != 0) {
                i++;  // Skip body
            }
            if (i < count) i++;  // Skip 'end'
            continue;
        }
        ...
        //     fprintf(out, "for (int %s = 0; %s; %s++", tokens[i+1].value, tokens[i+2].value, tokens[i+1].value);
        //     i++;
        } else if (strcmp(tokens[i].value, KEYWORD_INPUT) == 0) {
            fprintf(out, "char *%s;\n", tokens[i+2].value);
            fprintf(out, "printf(\"%s\");\n", tokens[i+1].value);
            fprintf(out, "scanf(\"%%%s\", %s);\n", tokens[i+2].type == TOKEN_VAR_INT ? "d" : tokens[i+1].type == TOKEN_VAR_FLOAT ? "f" : "s", tokens[i+2].value);
            i += 3;
        ...
        } else if (tokens[i].type == TOKEN_VAR_INT || tokens[i].type == TOKEN_VAR_FLOAT || tokens[i].type == TOKEN_VAR_STRING || tokens[i].type == TOKEN_VAR_BOOL) {
            fprintf(out, "%s", tokens[i].value);
            i++;
            ...
            fprintf(out, "%s;\n", tokens[i].value);
            i++;
        } else if (tokens[i].type == TOKEN_INT || tokens[i].type == TOKEN_FLOAT || tokens[i].type == TOKEN_BOOL) {
            fprintf(out, "%s", tokens[i].value);
            i++;
            ...
            if (tokens[i+1].type == TOKEN_STRING) {
                fprintf(out, "printf(\"%s\\n\");\n", tokens[i+1].value);
                i+=2;
            } else if (tokens[i+1].type == TOKEN_INT || tokens[i+1].type == TOKEN_OP) {
                fprintf(out, "printf(\"%%d\\n\", ");
                while (tokens[i+1].type == TOKEN_INT || tokens[i+1].type == TOKEN_OP) {
            ...
            if (tokens[i+3].type == TOKEN_STRING) {
                ...
                i+=4;
            } else if (tokens[i+3].type == TOKEN_INT || tokens[i+3].type == TOKEN_OP) {
                fprintf(out, "int %s = %s;\n", tokens[i+1].value, tokens[i+3].value);
                i+=4;
